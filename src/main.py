...
class StunClient(object):
    ...
    def check_nat_type(self, source_port=0):
        ...
        if source_port == 0:
            source_port = self.get_free_port(udp=True)
        self.logger.info("Checking NAT type for UDP with source ip %s, source port %s" % (self.source_ip, source_port))
...
def get_args():
    ...
    parser.add_argument("-i",
                        "--interface",
                        default=None,
                        help="source interface for client")
...
def main():
    ...
    if args.interface:
        source_ip = get_ip_address(args.interface)
    else:
        source_ip = args.source_ip
    stun_client = StunClient(source_ip, log_level=log_level)
    nat_type = stun_client.check_nat_type(args.source_port)
    ...
    print("NAT Type for UDP: [ %s ]" % nat_type_txt)
